...
def masked_mse_loss_with_global_normalizer(pred, target, mask):
    loss = F.mse_loss(pred, target, reduction="none") * mask
    local_numerator = loss.sum()
    local_denominator = mask.sum().to(dtype=local_numerator.dtype).clamp_min(1.0)
    ...
    global_denominator = global_denominator.clamp_min(1.0)

    world_size = torch.distributed.get_world_size()
    loss_for_backward = local_numerator * world_size / global_denominator
    ...
    return loss_for_backward + (global_loss - loss_for_backward).detach()
